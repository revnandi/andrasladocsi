...
if ($query->have_posts()): ?>
  <section class="flex flex-col mb-6 md:flex-row md:flex-wrap">
    <div class="flex items-center">
      <h2 class="text-lg font-display">Ongoing Projects</h2>
    ...
    <?php while ($query->have_posts()):
      $query->the_post(); ?>
      <div class="flex items-center italic al_hoverable_link group">
        <a href="<?php the_permalink() ?>" class="inline-block">
          <?php the_title(); ?>
        </a>
        <?php if (has_post_thumbnail()): ?>
          <div
            class="fixed hidden w-32 h-32 overflow-hidden border al_hoverable_link_image group-hover:block group md:col-span-5 aspect-wide-header aspect-square border-turquoise">
